...
func NewServer(cfg ServerConfig, rdb *redis.Client, jwtV *dealjwt.Verifier, h *handler.NotifyHandler, log zerolog.Logger) *gin.Engine {
	e := gin.New()
	e.Use(
	...
	p := e.Group("/api/v1/notifications")
	p.Use(middleware.JWTAuth(jwtV, rdb, log))
	{
		p.GET("/", h.GetNotifications)
		p.GET("/unread", h.GetUnreadCount)
		p.POST("/:id/read", h.MarkRead)
		p.POST("/read-all", h.MarkAllRead)
		p.POST("/device", h.RegisterDevice)
		p.GET("/preferences", h.GetPreferences)
		p.PATCH("/preferences", h.UpdatePreferences)
	}

	// Internal endpoint for service-to-service
	e.POST("/internal/notify", h.Send)
	return e
}
